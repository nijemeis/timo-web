...
export async function meResponse(u: User): Promise<MeResponse> {
  ...
  return {
    user: meUser(u),
    company: company ? { id: company.id, name: company.name, major: company.major, code: company.code, timezone: company.timezone, graceMinutes: company.graceMinutes } : null,
