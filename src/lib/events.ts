...
/**
 * Store a batch of phone pass events (idempotent on deviceId + seq) and run each new one through the engine, in
  ...
  await db.$transaction(
    async (tx) => {
      ...
      for (const ev of sorted) {
        ...
        else outcome = await apply(tx, user, company.id, beacon, ev.type, ev.minor, at);
...
async function apply(
  ...
  if (type === "exit" && !open && lastClosed?.status === "auto" && at >= lastClosed.checkInAt && at <= lastClosed.checkOutAt!) {
